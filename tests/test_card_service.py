...
"""
def test_check_delete_card(card_service, mock_db):
    # 1) Arrange
    mock_profile = Profile(
        UserID=1,
        Username="TestUsername1",
    ...
    mock_db.query.return_value.filter.return_value.first.return_value = existing_card

    # 2) Act
    result = card_service.delete_card(card_id = 1, username = "TestUsername1")
    ...
    # Assert that Card filtering was done correctly
    assert str(card_call) == str((Card.CardID == 1) & (Card.OwnerID == mock_profile.UserID)), \
        f"Expected {(Card.CardID == 1) & (Card.OwnerID == mock_profile.UserID)}, got {card_call}"
